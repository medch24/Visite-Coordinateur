...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // ===== GÉNÉRATION WORD PROFESSIONNELLE AMÉLIORÉE =====
    window.generateTeacherWordReport = async (evalId) => {
        if (typeof docx === 'undefined' || typeof saveAs === 'undefined') {
            ...
            alert("Erreur: Une librairie nécessaire n'a pas pu être chargée. Veuillez rafraîchir la page.");
            return;
        }
            ...
            Object.values(data.criteriaDetails || {}).forEach(cat => {
                ...
                cat.items.forEach(() => {
                    const rating = (data.rawCriteria && data.rawCriteria[`crit${critIndexSummary}`]) 
                        ...
                        : 0;
                    const score = Math.round((rating / 5) * cat.items[critIndexSummary % cat.items.length].points);
                    categoryTotal += score;
                    critIndexSummary++;
                });
